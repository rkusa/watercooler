...
var doc ="\
Usage: simulate.js.js <port> [--join=<join>] [--id=<id>] [--rate=<rate>] [--host=<host>] [--ui=<ui>]\n\
\n\
Options:\n\
  --id=<id>       The node's id.\n\
  --rate=<rate>   The gossip rate (per minute) [default: 12].\n\
  --host=<host>   The machines hostname.\n\
  --ui=<ui>       Whether to display the simulator's UI or not [default: true].\n\
"
...
var node = new watercooler.Node({
  id: opts['--id'],
  gossipRate: parseFloat(opts['--rate'], 10)
})
...
if (ui) {
  var index = 0
  debug.log = function() {
    arguments[0] = arguments[0].replace('watercooler:', '')
    // if (arguments[0].indexOf('gossip:') > -1) {
    //   return
    // }
